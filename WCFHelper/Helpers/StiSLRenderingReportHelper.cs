...
namespace WCFHelper
{
    public static class StiSLRenderingReportHelper
    {
                ...
                while (tr.Read())
                {
                    ...
                    switch (name)
                    {
                        ...
                        case "CollIndex":
                            ...
                            break;
                        case "IsCollapsed":
                            drillDownContainer.IsCollapsed = (tr.ReadString() == "1");
                            break;
                        case "PIndex":
                        ...
                        case "SDirec":
                            ...
                            break;
                        case "ControlPress":
                            drillDownContainer.IsControlPress = (tr.ReadString() == "1");
                            break;
                        case "ICollStates":
                            ...
                            while (tr.Read())
                            {
                                ...
                                if (tr.Depth == 2)
                                {
                                    ...
                                    currentTable = drillDownContainer.InteractionCollapsingStates[compName] as Hashtable;
                                }
                                else if (tr.Depth == 3)
                                {
        ...
        private static string StartDrillDownPage(StiDrillDownContainer drillDownContainer, DataSet ds)
        {
            drillDownContainer.Report.RegData(ds);

            drillDownContainer.Report.Compile();
            drillDownContainer.Report.Render(false);
            var compileReport = StiActivator.CreateObject(drillDownContainer.Report.CompiledReport.GetType()) as StiReport;
            ...
            var renderedPage = drillDownContainer.Report.RenderedPages[drillDownContainer.PageIndex];
            var interactionComp = renderedPage.Components[drillDownContainer.CompIndex];
        ...
        private static string StartCollapsing(StiDrillDownContainer drillDownContainer, DataSet ds)
        {
            drillDownContainer.Report.RegData(ds);
            ...
            var compileReport = drillDownContainer.Report.CompiledReport ?? drillDownContainer.Report;
        ...
        public static string CheckReportOnInteractions(StiReport report, bool useBaseReport)
        {
            ...
            if (!isSort)
            {
                ...
                {
                    if (string.IsNullOrEmpty(comp.Interaction.SortingColumn)) continue;
            ...
            var currentReport = report.CompiledReport ?? report;
            if (currentReport.InteractionCollapsingStates != null && currentReport.InteractionCollapsingStates.Count > 0)
            {
                ...
                foreach (string key in currentReport.InteractionCollapsingStates.Keys)
                {
                    ...
                    {
                        writer.WriteStartElement("_" + key1);
        ...
        {
            var report = DecodeXmlRequestFromUser(xml, previewDataSet);
            ...
            try
            {
                report.CompiledReport.IsSLPreviewDialogs = true;
                report.CompiledReport.Render(false);
                report.CompiledReport.IsSLPreviewDialogs = false;
            }
            catch
            {
                error = true;
        ...
        {
            var report = DecodeXmlPrepareRequestFromUserVariables(xml, previewDataSet);

            string result = string.Empty;
            if (report.CompiledReport != null)
            {
                report.CompiledReport.Dictionary.Connect();
                Stimulsoft.Report.Engine.StiVariableHelper.FillItemsOfVariables(report.CompiledReport ?? report);
        ...
        private static string GetPrepareRequestFromUserVariablesXml(StiReport compileReport)
        {
            ...
            foreach (StiVariable variable in compileReport.Dictionary.Variables)
            {
                ...
                foreach (var info in infos)
                {
                    ...
                    writer.WriteStartElement("ValueBinding");
                    writer.WriteValue(info.ValueBinding == null ? string.Empty : info.ValueBinding);
                    writer.WriteEndElement();
                }
        ...
        private static void ParseVariable(StiReport report, string variableName, StiRequestFromUserType type, XmlTextReader tr)
        {
            ...
            var compileReport = report.CompiledReport;
            if (compileReport == null) return;
        ...
        public static string InteractiveDataBandSelection(string xml, DataSet previewDataSet)
        {
            ...
            try
            {
                ...
                if (helper.Report.CompiledReport != null)
                {
                    ...
                    {
                        var band = helper.Report.CompiledReport.GetComponentByName(helper.DataBandNames[index]) as StiDataBand;
                        if (band != null)
        ...
        private static void RefreshInteractions(StiReport report)
        {
            ...
            var templateReport = report.CompiledReport ?? report;
            ...
            {
                var addedInteractions = new List<StiInteraction>();
                    ...
                    if (str == "ASC" || str == "DESC" || index == dataBand.Sort.Length - 1)
                    {
                        ...
                        if (sortStr.Length > 0)
                        {
                            ...
                            foreach (var interaction in list)
                            {
                                ...
                                if (str2 != sortStr) continue;

                                #region We have finded sorting string
                                interaction.SortingDirection = isAsc ? StiInteractionSortDirection.Ascending : StiInteractionSortDirection.Descending;
